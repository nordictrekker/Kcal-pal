...
create index on food_entries (user_id, consumed_at desc);
...
create table eight_sleep_daily (
  user_id uuid not null references auth.users(id) on delete cascade,
  date date not null,
  sleep_score int,
  total_sleep_min int,
...
create table apple_health_imports (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete cascade,
...
create index on apple_health_data (user_id, metric, recorded_at desc);
create unique index apple_health_data_unique
